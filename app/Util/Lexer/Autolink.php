...
class Autolink extends Regex
{
    ...
    public function __construct($tweet = null, $escape = true, $full_encode = false)
    {
        ...
        $host = getApHostUrl();
        $this->extractor = Extractor::create()->setMentions($this->mentions);
        $this->url_base_user = $host.'/user/';
        $this->url_base_list = $host.'/';
        $this->url_base_hash = $host.'/discover/tags/';
        $this->url_base_cash = $host.'/search?q=%24';
    }
    ...
    {
        $this->url_base_user = getApHostUrl() . $path;
        $this->target = null;
        $this->external = null;
        return $this;
    }
    ...
    {
        $this->url_base_hash = getApHostUrl() . $path;
        $this->target = null;
        $this->external = null;
        return $this;
    }
    ...
    public function autoLinkEntities($tweet = null, $entities = null)
    {
        ...
        foreach ($entities as $entity) {
            if (isset($entity['screen_name'])) {
                if(str_starts_with($entity['screen_name'], '@')) {
    ...
    public function autoLinkUsernamesAndLists($tweet = null)
    {
        ...
        if($this->autolinkActiveUsersOnly == true) {
        	$entities = \Hyperf\Collection\collect($entities)
        		->filter(function($entity) {
        			return Helper::getExistsMention($entity['screen_name'], $this->mentions);
        		})
        		->toArray();
        }
    ...
    public function linkToUrl($entity)
    {
        ...
        // $linkText = $this->escapeHTML(strlen($entity['url']) > 20 ? '点击查看' : $entity['url']);
        $linkText = $this->escapeHTML(mb_strimwidth($entity['url'],0,30,'...'));
...
//        $attributes['href'] = $this->url_base_hash.$entity['hashtag'].'?src=hash';
    ...
    public function linkToMentionAndList($entity)
    {
        ...
        if($this->autolinkActiveUsersOnly == true) {
        	if(!Helper::getExistsMention($screen_name, $this->mentions)) {
        		return str_starts_with($screen_name, '@') ? $screen_name : "@{$screen_name}";
        	}
        }

//        if (!empty($entity['list_slug'])) {
//            // Replace the list and username
//            $linkText = str_starts_with($screen_name, '@') ? $screen_name : '@'.$screen_name;
...
//        } else {
            ...
            $fullScreenName = $this->getFullScreenName($screen_name);
            $url = $this->url_base_user . $fullScreenName;
//        }
        if (!empty($class)) {
    ...
    public function getFullScreenName($screen_name)
    {
        $screen_name = trim($screen_name, '@');
        $mention = Helper::getExistsMention($screen_name, $this->mentions);
        return $mention['acct'] ?? $screen_name;
    }
