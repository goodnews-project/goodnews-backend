    ...
    public function extractMentionedScreennames($tweet = null)
    {
        ...
        foreach ($mentionsWithIndices as $mentionWithIndex) {
        	if($this->activeUsersOnly == true) {
        		if(!Helper::getExistsMention($mentionWithIndex['screen_name'], $this->mentions)) {
        			continue;
        		}
        	}
...
//            $screenname = trim($all[0]) == '@'.$username[0] ? $username[0] : trim($all[0]);
            $screenname = trim($all[0]) == '@'.$username[0] ? $username[0] : StringUtils::substr($all[0], 1);
            if($this->activeUsersOnly == true) {
        		if(!Helper::getExistsMention($screenname, $this->mentions)) {
        			continue;
        		}
        	}
