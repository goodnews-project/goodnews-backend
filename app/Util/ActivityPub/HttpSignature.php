...
class HttpSignature
{
    ...
    public static function sign(Account $account, $url, $body = false, $addlHeaders = [])
    {
        if ($body) {
            $digest = self::_digest($body);
        }
        $user = $account;
        $accountUris = UrisService::generateURIsForAccount($account->username);
        $keyId = $accountUris['publicKeyURI'];
    ...
    {
        $actor = AccountService::getActor();
        $keyId = UrisService::generateURIsForAccount($actor->username)['publicKeyURI'];
    ...
    public static function verify($publicKey, $signatureData, $inputHeaders, $path)
    {
        $newHeaders = [];
        foreach ($inputHeaders as $k => $v) {
            $newHeaders[strtolower($k)] = $v;
        }
        $inputHeaders = $newHeaders;
//        $digest = 'SHA-256=' . base64_encode(hash('sha256', $body, true));
        ...
        $headersToSign['host'] = \Hyperf\Support\env('AP_HOST');
    ...
    public static function headersToCurlArray($headers)
    {
